...
impl LocalBlockchain {
    pub(crate) async fn run(
        ...
        let private_keys_path = resolve_repo_relative_path(PRIVATE_KEYS_PATH).ok_or_else(|| {
            let searched = resolve_repo_relative_candidates(PRIVATE_KEYS_PATH)
                .into_iter()
                .map(|candidate| candidate.display().to_string())
                .collect::<Vec<_>>()
                .join(", ");
            format!(
                "Failed to read private keys file '{}': file not found (searched: {})",
                PRIVATE_KEYS_PATH, searched
            )
        })?;
