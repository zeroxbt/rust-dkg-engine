...
use dkg_blockchain::{B256, BlockchainError, BlockchainManager};
use dkg_domain::{
    AccessPolicy, ParsedUal, UalParseError, Visibility, construct_knowledge_collection_onchain_id,
...
#[derive(Debug, Error)]
pub(crate) enum ParanetError {
    #[error("Invalid paranet UAL: {0}")]
    InvalidParanetUal(#[from] UalParseError),
    #[error("Paranet UAL must include knowledge asset ID")]
    MissingKnowledgeAssetId,
    #[error("Paranet does not exist: {paranet_ual}")]
    ParanetDoesNotExist { paranet_ual: String },
    #[error("Knowledge collection not registered in paranet")]
    KnowledgeCollectionNotRegistered,
    #[error("Failed to check paranet existence: {source}")]
    CheckParanetExistence {
        #[source]
        source: BlockchainError,
    },
    #[error("Failed to get access policy: {source}")]
    GetAccessPolicy {
        #[source]
        source: BlockchainError,
    },
    #[error("Failed to check KC registration in paranet: {source}")]
    CheckKcRegistration {
        #[source]
        source: BlockchainError,
    },
    #[error("Failed to get permissioned nodes: {source}")]
...
pub(crate) async fn select_shard_peers_for_paranet(
...
) -> Result<Vec<PeerId>, ParanetError> {
    let paranet_id = parse_paranet_id(paranet_ual)?;

    let exists = blockchain_manager
        .paranet_exists(&target_ual.blockchain, paranet_id)
        .await
        .map_err(|source| ParanetError::CheckParanetExistence { source })?;
    if !exists {
        return Err(ParanetError::ParanetDoesNotExist {
            paranet_ual: paranet_ual.to_string(),
        });
    }
    ...
    let policy = blockchain_manager
        ...
        .await
        .map_err(|source| ParanetError::GetAccessPolicy { source })?;

    let kc_registered = is_target_kc_registered(blockchain_manager, target_ual, paranet_id).await?;
    if !kc_registered {
        return Err(ParanetError::KnowledgeCollectionNotRegistered);
    }
...
pub(crate) async fn determine_get_visibility_for_paranet(
    ...
    let paranet_id = match parse_paranet_id(paranet_ual) {
        ...
        Err(ParanetError::InvalidParanetUal(error)) => {
            tracing::debug!(
        ...
        Err(ParanetError::MissingKnowledgeAssetId) => {
            tracing::debug!(
                paranet_ual = %paranet_ual,
        ...
        Err(error) => {
            tracing::debug!(
                paranet_ual = %paranet_ual,
                error = %error,
                "Failed to resolve paranet id, using Public visibility"
            );
            return Visibility::Public;
        }
    };
    ...
    let kc_registered =
        match is_target_kc_registered(blockchain_manager, target_ual, paranet_id).await {
            Ok(kc_registered) => kc_registered,
            Err(error) => {
                tracing::debug!(
                    paranet_id = %paranet_id,
                    error = %error,
...
fn parse_paranet_id(paranet_ual: &str) -> Result<B256, ParanetError> {
    let paranet_parsed = parse_ual(paranet_ual)?;
    let ka_id = paranet_parsed
        .knowledge_asset_id
        .ok_or(ParanetError::MissingKnowledgeAssetId)?;
...
async fn is_target_kc_registered(
    ...
    blockchain_manager
        ...
        .await
        .map_err(|source| ParanetError::CheckKcRegistration { source })
}
...
async fn permissioned_peer_ids(
    ...
    paranet_id: B256,
) -> Result<HashSet<PeerId>, ParanetError> {
    let permissioned_nodes = blockchain_manager
        .get_permissioned_nodes(blockchain_id, paranet_id)
        .await
        .map_err(|source| ParanetError::GetPermissionedNodes { source })?;
