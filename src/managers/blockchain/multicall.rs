...
#[derive(Clone)]
pub(crate) struct MulticallRequest {
...
impl MulticallRequest {
    ...
    pub(crate) fn new(target: Address, call_data: Bytes) -> Self {
        Self {
            target,
            allow_failure: true,
            call_data,
    ...
    #[allow(dead_code)]
    pub(crate) fn new_required(target: Address, call_data: Bytes) -> Self {
        Self {
            target,
            allow_failure: false,
            call_data,
    ...
    pub(crate) fn into_call3(self) -> Multicall3::Call3 {
        Multicall3::Call3 {
            target: self.target,
...
#[derive(Clone)]
pub(crate) struct MulticallResult {
...
impl MulticallResult {
    ...
    pub(crate) fn from_result(result: &Multicall3::Result) -> Self {
        Self {
    ...
    pub(crate) fn as_bytes32_hex(&self) -> Option<String> {
        if !self.success || self.data().len() < 32 {
            return None;
        }
    ...
    pub(crate) fn as_knowledge_assets_range(&self) -> Option<(u64, u64, Vec<u64>)> {
        let data = self.data();
...
/// ```
pub(crate) struct MulticallBatch {
...
impl MulticallBatch {
    ...
    pub(crate) fn add(&mut self, request: MulticallRequest) {
    ...
    pub(crate) fn into_chunks(self) -> impl Iterator<Item = Vec<Multicall3::Call3>> {
        self.requests
            .into_iter()
...
pub(crate) mod encoders {
    ...
    pub(crate) fn encode_uint256_call(selector: [u8; 4], value: u128) -> Bytes {
