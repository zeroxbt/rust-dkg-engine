...
async fn main() {
    ...
    let (network_manager, repository_manager, blockchain_manager) =
        initialize_managers(config.managers).await;

    initialize_dev_environment(Arc::clone(&blockchain_manager)).await;
    ...
    let (http_api_router, rpc_router, blockchain_event_handler) =
        initialize_handlers(config.http_api, Arc::clone(&context));
...
fn display_ot_node_ascii_art() {
    ...
    if let Ok(environment) = std::env::var("NODE_ENV") {
        ...
        panic!("NODE_ENV environment variable not set");
    }
}
...
async fn initialize_managers(
    ...
    (network_manager, repository_manager, blockchain_manager)
}
...
fn initialize_handlers(
    ...
    let http_api_router = HttpApiRouter::new(http_api_config, Arc::clone(&context));
    let rpc_router = RpcRouter::new();
    let blockchain_event_handler = BlockchainEventHandler::new(
        Arc::clone(context.blockchain_manager()),
        Arc::clone(context.repository_manager()),
    );
...
async fn initialize_dev_environment(blockchain_manager: Arc<BlockchainManager>) {
    for blockchain in blockchain_manager.get_blockchain_names() {
        if std::env::var("NODE_ENV").unwrap() == "development" {
