...
impl ProofChallengeRepository {
    ...
    pub async fn get_latest(&self, blockchain_id: &str) -> Result<Option<Model>> {
        Ok(Entity::find()
    ...
    pub async fn find_expired(&self, cutoff: i64, limit: u64) -> Result<Vec<Model>> {
        if limit == 0 {
            return Ok(Vec::new());
        }
    ...
    pub async fn delete_by_keys(&self, entries: &[Model]) -> Result<u64> {
        if entries.is_empty() {
            return Ok(0);
        }
